...
To keep things simple, we'll use a community-developed client lirbary called `airrecord` which has an ActiveRecord-like interface to the Airtable API. This will allow us to use user-friendly abstractions to obtain records and represent their relationships programmatically within our app.
...
Now that we have a model, we can simply reference it in our script. I've created a copy of this script called `send_agenda.rb` to ensure that there's an example available in the repo at each relevant step (you could keep building off the original instead, if you'd like).

```ruby
require_relative 'airtable/company'
```

To find the records, we simply call our macro, iterate over the objects, then use that information to send our message. We can reference any field for a given record as a hash using a string key corresponding to the column name in Airtable (e.g. `record["name"]` for "Name")
...
Building all this plumbing means nothing if we don't have some automation, right? We want to send a text message with our selected set of tasks each morning around 8:00AM. We'll do this by deploying our application to Heroku and then using the free Heroku scheduler to send a message.
...
Once deployed, check your work by spinning up one-off dyno to run your code.

```
heroku run "ruby send_agenda.rb"
```

Finally, we'll add the Heroku Scheduler add-on to our app. This will allow us to specify when and how often our app should run. Add a job to publish our agenda daily at 3:00pm UTC (8:00AM PDT), executing the following command:
